...
const float kInternalRate = 1.0f / 6.0f;
...
class Renderer {
 public:
  Renderer() : clock_phase_(0.0f), sample_(0.0f), next_sample_(0.0f) {
    synth_.Init();
  ...
  float RenderSample() {
    ...
    clock_phase_ += kInternalRate;
    if (clock_phase_ >= 1.0f) {
      clock_phase_ -= 1.0f;
      const float reset_time = clock_phase_ / kInternalRate;
      float excitation;
      float new_sample;
      synth_.Render(0.0f, 1.0f, &excitation, &new_sample, 1);
...
int main(int argc, char** argv) {
  if (argc != 3) {
    std::fprintf(stderr, "usage: %s PLAN OUTPUT.wav\n", argv[0]);
    return 2;
  }
  ...
  std::printf("rendered %s (%.3f s, raw peak %.4f, fixed gain %.2f)\n",
