...
COMMAND=""
if [[ $* == *clean* ]]; then
    COMMAND="$COMMAND make -f rings/makefile clean &&"
fi
COMMAND="$COMMAND make -f rings/makefile"
if [[ $* == *size* ]]; then
    COMMAND="$COMMAND && make -f rings/makefile size"
fi
if [[ $* == *wav* ]]; then
...
if [[ $* == *wav* ]]; then
    WAVFILE="`pwd`/../build/rings/rings.wav"
    echo
    echo "PLAYING:"
    echo "  `ls -l "$WAVFILE"`"
    echo
