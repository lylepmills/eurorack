#!/usr/bin/env python3
"""Build linker-prunable, CPU-probed firmware for each terrain equation case."""
...
FIXED_DEFINE = "#define PLAITS_TERRAIN_BENCH_FIXED_CASE -1"
CASE_NAMES = (
    "original-terrain-1",
    "sampled-grid-4kb",
    "soft-rings",
...
def main() -> int:
    ...
    parser.add_argument("--led-only", action="store_true")
    ...
    for case in selected:
        ...
        with tempfile.TemporaryDirectory(prefix=f"terrain-equation-{case:02d}-") as temp:
            ...
            fixed_source.write_text(
                source.replace(FIXED_DEFINE,
                               f"#define PLAITS_TERRAIN_BENCH_FIXED_CASE {case}"),
                encoding="utf-8",
            )
            ...
            command = [
                ...
                str(package),
                "--hardware",
                "--cpu-probe" if args.led_only else "--cpu-probe-aux",
                "--output",
                str(output),
            ]
            print(f"\n== {case:02d} {name} ==", flush=True)
            subprocess.run(command, check=True)

    print(f"\nBuilt {len(selected)} diagnostic firmware file(s) in {args.output_dir}")
    return 0
