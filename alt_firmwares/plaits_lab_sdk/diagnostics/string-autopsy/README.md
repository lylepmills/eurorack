...
`build --hardware --cpu-probe-aux` (per-section reports exist only on the tone
channel, so the LED meter alone will not do here); the AUX readout cycles
through identity-beacon
