"""Recipe-baked word banks for the Natural Voice engine.
...
a generated header the firmware compiles in. Natural Voice does NOT take its
...
The frame format is NVH1, 23 bytes, as produced by the WORLD-vocoder encoder
...
def validate_natural_voice_banks(value: Any) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != {"customBanks"}:
        raise ValueError("naturalVoiceBanks must contain customBanks")
    custom = value["customBanks"]
    if not isinstance(custom, list) or not 1 <= len(custom) <= MAX_BANKS:
        raise ValueError(
            f"Natural Voice must contain between one and {MAX_BANKS} banks")

    normalized = []
    for bank in custom:
        if (not isinstance(bank, dict)
                or set(bank) != {"words", "wordBoundaries", "frameData"}):
            raise ValueError("a Natural Voice bank has an unsupported shape")
        words = bank["words"]
        ...
        if (not isinstance(words, list) or not 1 <= len(words) <= MAX_WORDS
            ...
            raise ValueError(
                f"a Natural Voice bank must contain 1 through {MAX_WORDS} "
                "named words")
        if not isinstance(encoded, str):
            raise ValueError("a Natural Voice bank is missing its frame data")
        try:
            packed = base64.b64decode(encoded, validate=True)
        except (ValueError, TypeError) as error:
            raise ValueError(
                "a Natural Voice bank contains invalid frame data") from error
        if not packed or len(packed) % BYTES_PER_FRAME:
            raise ValueError(
                "a Natural Voice bank contains misaligned frame data")
        frame_count = len(packed) // BYTES_PER_FRAME
        if frame_count > MAX_FRAMES:
            raise ValueError(
                f"a Natural Voice bank exceeds {MAX_FRAMES} frames")
        if (not isinstance(boundaries, list)
            ...
            raise ValueError(
                "a Natural Voice bank contains invalid word boundaries")
...
def render_natural_voice_config(value: dict[str, Any] | None) -> str:
    header = [
        "// Generated by natural_voice_banks.py. Do not edit.",
        "#ifndef PLAITS_DSP_NATURAL_VOICE_RECIPE_CONFIG_H_",
        "#define PLAITS_DSP_NATURAL_VOICE_RECIPE_CONFIG_H_",
    ]
    if value is None:
        return "\n".join(header + [
    ...
    means: list[int] = []
    lines = header + [
        "#define PLAITS_HAS_CUSTOM_NATURAL_VOICE_BANKS 1",
        "#include <stdint.h>",
        "namespace plaits {",
        "namespace natural_voice_recipe {",
    ]
    ...
    for bank_index, bank in enumerate(custom):
        ...
        for word_index, word in enumerate(bank["words"]):
            ...
            cursor += count
    lines.append("};")
    lines.append("}  // namespace natural_voice_recipe")
...
def bank_flash_bytes(value: dict[str, Any] | None) -> int:
    ...
    Natural Voice frames are 23 bytes at 40 Hz -- 920 B per second of
