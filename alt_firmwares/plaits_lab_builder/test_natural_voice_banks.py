"""Tests for the Natural Voice recipe bank contract."""
...
from natural_voice_banks import (
    BYTES_PER_FRAME,
    FRAME_STRUCT,
    LPC_ORDER,
    MAX_BANKS,
    ...
    MAX_WORDS,
    bank_flash_bytes,
    render_natural_voice_config,
    validate_natural_voice_banks,
)
...
class ValidationTests(unittest.TestCase):
    ...
    def test_rejects_misaligned_frame_data(self):
        ...
        with self.assertRaises(ValueError):
            validate_natural_voice_banks(recipe(broken))
    ...
    def test_rejects_too_many_banks(self):
        with self.assertRaises(ValueError):
            validate_natural_voice_banks(recipe(*[bank()] * (MAX_BANKS + 1)))

    def test_rejects_an_empty_recipe(self):
        with self.assertRaises(ValueError):
            validate_natural_voice_banks({"customBanks": []})

    def test_rejects_too_many_words(self):
        words = tuple(f"w{i}" for i in range(MAX_WORDS + 1))
        with self.assertRaises(ValueError):
            validate_natural_voice_banks(recipe(bank(words)))
    ...
    def test_rejects_a_bank_over_the_frame_ceiling(self):
        ...
        over = {"words": ["long"], "wordBoundaries": [0, MAX_FRAMES + 1],
                "frameData": base64.b64encode(packed).decode("ascii")}
        with self.assertRaises(ValueError):
            validate_natural_voice_banks(recipe(over))
...
class MeanTractTests(unittest.TestCase):
    ...
    def test_mean_is_derived_from_the_frames(self):
        lars = [7] * LPC_ORDER
        result = validate_natural_voice_banks(recipe(bank(lars=lars)))
    ...
    def test_silent_frames_do_not_drag_the_mean(self):
        ...
        entry = {"words": ["w"], "wordBoundaries": [0, 2],
                 "frameData": base64.b64encode(packed).decode("ascii")}
        result = validate_natural_voice_banks(recipe(entry))
        self.assertEqual(result["customBanks"][0]["meanLar"],
                         [10] * LPC_ORDER)
    ...
    def test_mean_is_not_accepted_from_the_client(self):
        ...
        with self.assertRaises(ValueError):
            validate_natural_voice_banks(recipe(entry))


class RenderTests(unittest.TestCase):
    def test_absent_banks_render_the_off_switch(self):
        text = render_natural_voice_config(None)
        self.assertIn("#define PLAITS_HAS_CUSTOM_NATURAL_VOICE_BANKS 0", text)
        self.assertNotIn("kBankFrames", text)

    def test_rendered_config_carries_every_frame(self):
        value = validate_natural_voice_banks(
            recipe(bank(("one", "two")), bank(("three",))))
        text = render_natural_voice_config(value)
        self.assertIn("#define PLAITS_HAS_CUSTOM_NATURAL_VOICE_BANKS 1", text)
    ...
    def test_word_names_cannot_break_out_of_a_comment(self):
        value = validate_natural_voice_banks(recipe(bank(("a */ evil",))))
        text = render_natural_voice_config(value)
...
class FlashCostTests(unittest.TestCase):
    def test_cost_is_reported_at_the_real_frame_size(self):
        value = validate_natural_voice_banks(recipe(bank(("a", "b"))))
