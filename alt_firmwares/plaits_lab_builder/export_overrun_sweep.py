...
def write_scene_table(path: Path, engines: list[str], output: Path) -> None:
    """scene_table.h for plaits/scene_check.h, and scenes.json for the host."""
    scenes = json.loads(path.read_text())
    rows = []
    for scene in scenes:
        aux, subosc = AUX_MODES[scene["aux"]]
        rows.append(
            f"  {{ {engines.index(scene['engine'])}, {scene['harmonics']:.4f}f, "
            f"{scene['timbre']:.4f}f, {scene['morph']:.4f}f, "
            f"{scene['twist']:.4f}f, {scene['note']:.2f}f, {aux}, {subosc}, "
            f"{1 if scene['triggered'] else 0} }}")
    (output / "scene_table.h").write_text(
        "// Generated by export_overrun_sweep.py --scenes.\n"
        f"#define PLAITS_SCENE_COUNT {len(rows)}\n"
        "#define PLAITS_SCENES { \\\n" + ", \\\n".join(rows) + " \\\n}\n",
        encoding="utf-8")
...
def main() -> None:
    ...
    parser.add_argument("--scenes", type=Path,
                        help="JSON list of fixed scenes for plaits/"
                             "scene_check.h (implies the scene-check build)")
    ...
    if args.scenes:
        flags = ("-DPLAITS_SCENE_CHECK=1 -include "
                 f"{args.output.absolute() / 'scene_table.h'}")
    elif args.threshold:
        flags = "-DPLAITS_THRESHOLD_LADDER=1"
    else:
        flags = (f"-DPLAITS_OVERRUN_SWEEP=1 "
    ...
    if args.scenes:
        write_scene_table(args.scenes, engines, args.output)
